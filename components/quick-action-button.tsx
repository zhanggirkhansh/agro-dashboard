...
export default function QuickActionButton() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-[#e6ebdf] bg-white shadow-xl">
          {ACTIONS.map(({ label, href, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 px-4 py-3 text-sm text-[#1f2937] hover:bg-[#f3f6ef] hover:text-[#1f4d3a]"
            >
              <Icon size={15} className="shrink-0 text-[#2f6a4f]" />
              {label}
            </Link>
          ))}
        </div>
